...
use crate::libsuzu::graphics::object::shape::MeshShape;
use crate::libsuzu::graphics::object::sub_screen;
use crate::libsuzu::graphics::object::tile_batch::*;
...
impl TableFrame {
    ...
    pub fn contains_at(&self, grid_position: numeric::Vector2u, point: numeric::Point2f) -> bool {
...
impl Node2D for TableFrame {
    impl_node2d_for_wrapped! {tile_batch}
}
...
pub struct SelectButton {
    canvas: SubScreen,
    button_texture: Box<dyn Transform>,
    button_toggle: bool,
}
...
impl Drawable for SelectButton {
    fn draw(&mut self, ctx: &mut ggez::Context) -> ggez::GameResult<()> {
        if self.is_visible() {
...
impl Node2D for SelectButton {
    impl_node2d_for_wrapped! {canvas}
}

impl Transform for SelectButton {
...
impl Drawable for TextButtonTexture {
    fn draw(&mut self, ctx: &mut ggez::Context) -> ggez::GameResult<()> {
        if self.is_visible() {
...
impl Node2D for TextButtonTexture {
...
impl Transform for TextButtonTexture {
...
impl Drawable for TileBatchFrame {
    fn draw(&mut self, ctx: &mut ggez::Context) -> ggez::GameResult<()> {
        if self.is_visible() {
            self.tile_batch.draw(ctx).unwrap()
        }
...
impl<D> Drawable for ScrollableWindow<D>
where
    D: Scrollable,
{
    fn draw(&mut self, ctx: &mut ggez::Context) -> ggez::GameResult<()> {
...
impl<D> Node2D for ScrollableWindow<D>
where
    D: Scrollable,
{
    impl_node2d_for_wrapped! {canvas}
...
impl<D> Transform for ScrollableWindow<D>
where
    D: Scrollable,
{
    impl_transform_object_for_wrapped! {canvas}
...
impl Node2D for GraphDrawer {
    impl_node2d_for_wrapped! {canvas}
}

impl Transform for GraphDrawer {
    impl_transform_object_for_wrapped! {canvas}
}
...
impl Drawable for PauseScreenSet {
    fn draw(&mut self, ctx: &mut ggez::Context) -> ggez::GameResult<()> {
        if self.is_visible() {
...
pub struct CheckBox {
    frame: TableFrame,
    check_texture: Box<dyn Transform>,
    drwob_essential: DrawableEss,
    is_checked: bool,
}
...
impl CheckBox {
    pub fn new<'a>(
        ctx: &mut SuzuContext<'a>,
        pos_rect: numeric::Rect,
        mut check_texture: Box<dyn Transform>,
        is_checked: bool,
        depth: i8,
    ) -> Self {
...
impl Drawable for FramedButton {
    fn draw(&mut self, ctx: &mut ggez::Context) -> ggez::GameResult<()> {
        if self.is_visible() {
