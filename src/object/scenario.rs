...
use crate::libsuzu::{graphics::drawable::*, sound::SoundHandler};
...
impl Drawable for ScenarioTachie {
...
impl Node2D for ChoiceBox {
    impl_node2d_for_wrapped! {canvas}
}

impl Transform for ChoiceBox {
...
impl Scenario {
	...
	save_data: Option<&ScenarioSceneSaveData>,
		...
		.parse::<toml::Value>()
		.expect("Failed to parse toml file")	    
	} else if file_path ==  "/scenario/tutorial/task/r1.toml" {
	    RETURNING_TUTORIAL_SCEANRIO
		.parse::<toml::Value>()
		.expect("Failed to parse toml file")	    
	} else {
...
impl TextBox {
    pub fn new<'a>(
        ...
        let mut line_arrow = Texture::new(
...
impl Drawable for ScenarioEvent {
    fn draw(&mut self, ctx: &mut ggez::Context) -> ggez::GameResult<()> {
        if self.is_visible() {
...
const RETURNING_TUTORIAL_SCEANRIO: &str = "first-scenario-id = 10
...
[scenario-group.tachie-data]
   ...
   [[scenario-group.text]]
   text = \"\"\"*助言*
お客さんが来ました。次に返却業務について簡単に説明します。
まず、貸出本をクリックし本の名前をメモしましょう。
次にお客さんの名前と貸出期限を質問しましょう。
鈴奈庵帳簿をクリックして開き、お客さんの貸出記録を確認します。
そして本の状態を観察し中央の記録欄二段目の状態を追記します。
ページ左部下段の欄をクリックして押印します。
その後は本を返却箱にドラッグすれば返却業務は完了です。
また、お客さんの中には延滞してしまっている方がいます。
それに気がついた際は帳簿の返却日をクリックすると
延滞を指摘するメニューが表示されるので可能な限り指摘しましょう。
以上で説明を終わります。
\"\"\"
...
id = 12
next-id = 12
opecode = \"TutorialFinishReturning\"
background = \"SightBackground1\"";
