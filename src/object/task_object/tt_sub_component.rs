...
use crate::libsuzu::graphics::object::shadow::*;
...
impl Drawable for HoldDataVText {
...
impl Node2D for HoldDataVText {
    impl_node2d_for_wrapped! {vtext}
}

impl Transform for HoldDataVText {
    impl_transform_object_for_wrapped! {vtext}
}
...
pub trait OnDesk: Transform + Clickable {
...
impl OnDeskTexture {
    pub fn new(ctx: &mut ggez::Context, mut obj: Texture, on_desk_type: OnDeskType) -> Self {
...
impl Drawable for OnDeskTexture {
    fn draw(&mut self, ctx: &mut ggez::Context) -> ggez::GameResult<()> {
        if self.is_visible() {
...
impl Node2D for OnDeskTexture {
    impl_node2d_for_wrapped! {canvas}
}

impl Transform for OnDeskTexture {
...
pub struct OnDeskBook {
    ...
    book_texture: Texture,
    scratch_texture: Option<Texture>,
    shadow: ShadowShape,
    title: VerticalText,
    canvas: SubScreen,
}
...
impl OnDeskBook {
    pub fn new<'a>(
        ...
        let mut scratch_texture = match info.get_condition() {
            ...
            BookCondition::Fair => Some(Texture::new(
            ...
            BookCondition::Bad => Some(Texture::new(
...
pub struct SignFrame {
    ...
    borrowing_sign: Option<Texture>,
    returning_sign: Option<Texture>,
...
impl SignFrame {
    ...
    pub fn sign_borrowing_frame(&mut self, ctx: &mut SuzuContext) {
        let mut sign_texture = Texture::new(
...
impl BorrowingRecordBookPage {
    ...
    pub fn new_empty<'a>(
        ...
        BorrowingRecordBookPage {
            ...
            drwob_essential: DrawableEss::new(true, 0),
        }
    }
...
impl Drawable for BorrowingRecordBookPage {
    fn draw(&mut self, ctx: &mut ggez::Context) -> ggez::GameResult<()> {
        if self.is_visible() {
...
pub struct BorrowingRecordBook {
    ...
    current_page: usize,
    next_page_ope_mesh: Texture,
    prev_page_ope_mesh: Texture,
...
impl BorrowingRecordBook {
	...
	for page in self.pages.iter() {
	    if !page.sign_frame.retuning_signing_is_done() &&
		!page.sign_frame.borrowing_signing_is_done() &&
		page.contains_book_id(book_info.get_unique_id()) {
		    return;
		}
	}
...
impl Drawable for BorrowingRecordBook {
    #[inline(always)]
    fn draw(&mut self, ctx: &mut ggez::Context) -> ggez::GameResult<()> {
        if self.is_visible() {
    ...
    fn virtual_key_event(
        ...
        _event_type: crate::libsuzu::device::KeyboardEvent,
        _vkey: crate::libsuzu::device::VirtualKey,
    ) {
        // Nothing
    }
    fn mouse_button_event(
        &mut self,
        _ctx: &mut ggez::Context,
        _event_type: crate::libsuzu::device::MouseButtonEvent,
...
impl Node2D for BorrowingRecordBook {
    impl_node2d_for_wrapped! {canvas}
}

impl Transform for BorrowingRecordBook {
    impl_transform_object_for_wrapped! {canvas}
}
...
pub struct TaskManualBook {
    redraw_request: DrawRequest,
    pages: Vec<Box<dyn Drawable>>,
    go_left_texture: Texture,
    go_right_texture: Texture,
    background: Texture,
...
impl TaskManualBook {
    pub fn new<'a>(ctx: &mut SuzuContext<'a>, rect: numeric::Rect, depth: i8) -> Self {
        let mut borrowing_flow = Box::new(Texture::new(
        ...
        let mut return_flow = Box::new(Texture::new(
        ...
        let mut title_details = Box::new(Texture::new(
        ...
        let pages = vec![
            borrowing_flow as Box<dyn Drawable>,
            return_flow as Box<dyn Drawable>,
            title_details as Box<dyn Drawable>,
        ];

        let mut left = Texture::new(
            ctx.ref_texture(TextureID::GoNextPageLeft),
...
impl Drawable for TaskManualBook {
    fn draw(&mut self, ctx: &mut ggez::Context) -> ggez::GameResult<()> {
        if self.is_visible() {
            if self.redraw_request != DrawRequest::Skip {
                self.redraw_request = DrawRequest::Skip;
...
impl Node2D for TaskManualBook {
    impl_node2d_for_wrapped! {canvas}
}

impl Transform for TaskManualBook {
    impl_transform_object_for_wrapped! {canvas}
}
