...
use crate::roundup2f;
use crate::libsuzu::{core::Clock, graphics::drawable::*};
...
impl Drawable for SuzunaStatusMainPage {
...
impl AdEntry {
    pub fn new<'a>(
        ...
        let choice_box_texture = Box::new(Texture::new(
...
impl ScenarioAdPage {
    ...
    pub fn all_checks_are_cleared<'a>(&mut self, _ctx: &mut SuzuContext<'a>) -> bool {
        for ad_type in vec![
            SuzunaAdType::AdPaper,
            SuzunaAdType::Chindon,
            SuzunaAdType::ShopNobori,
...
impl Drawable for ScenarioAdPage {
    fn draw(&mut self, ctx: &mut ggez::Context) -> ggez::GameResult<()> {
        if self.is_visible() {
...
impl Drawable for ScenarioAgencyPage {
    fn draw(&mut self, ctx: &mut ggez::Context) -> ggez::GameResult<()> {
        if self.is_visible() {
            self.header_text.draw(ctx)?;
            for (_, entry) in self.ad_table.iter_mut() {
...
impl SuzunaStatusScreen {
    pub fn new<'a>(
        ...
        let mut background_texture = Texture::new(
...
impl Drawable for SuzunaStatusScreen {
    fn draw(&mut self, ctx: &mut ggez::Context) -> ggez::GameResult<()> {
        if self.is_visible() {
            if self.redraw_request.is_not_skip() {
...
pub trait StackableWindow: Transform {
...
impl<Msg> Drawable for WindowStack<Msg> {
    fn draw(&mut self, ctx: &mut ggez::Context) -> ggez::GameResult<()> {
        if self.is_visible() {
...
impl Drawable for WeekScheduleWindow {
    fn draw(&mut self, ctx: &mut ggez::Context) -> ggez::GameResult<()> {
        if self.is_visible() {
            sub_screen::stack_screen(ctx, &self.canvas);
...
impl Node2D for WeekScheduleWindow {
    impl_node2d_for_wrapped! {canvas}
}

impl Transform for WeekScheduleWindow {
...
impl Drawable for ScheduleSelectWindow {
    fn draw(&mut self, ctx: &mut ggez::Context) -> ggez::GameResult<()> {
        if self.is_visible() {
            sub_screen::stack_screen(ctx, &self.canvas);
...
impl Node2D for ScheduleSelectWindow {
    impl_node2d_for_wrapped! {canvas}
}

impl Transform for ScheduleSelectWindow {
    impl_transform_object_for_wrapped! {canvas}
}
...
impl Drawable for ScenarioSchedPage {
    fn draw(&mut self, ctx: &mut ggez::Context) -> ggez::GameResult<()> {
        if self.is_visible() {
            self.header_text.draw(ctx)?;
