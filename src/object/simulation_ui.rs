...
use crate::libsuzu::graphics::object::shape as tshape;
...
impl Drawable for ScenarioMenuContents {
    fn draw(&mut self, ctx: &mut ggez::Context) -> ggez::GameResult<()> {
        if self.is_visible() {
...
impl Drawable for ScenarioMenu {
    fn draw(&mut self, ctx: &mut ggez::Context) -> ggez::GameResult<()> {
