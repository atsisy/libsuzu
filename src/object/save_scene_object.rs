...
impl DrawableSaveEntry {
    pub fn new<'a>(
        ...
        let mut background = Texture::new(
...
impl Drawable for DrawableSaveEntry {
...
impl Node2D for DrawableSaveEntry {
    impl_node2d_for_wrapped! {canvas}
}

impl Transform for DrawableSaveEntry {
...
impl Drawable for SaveEntryTable {
    fn draw(&mut self, ctx: &mut ggez::Context) -> ggez::GameResult<()> {
        if self.is_visible() {
	    if self.redraw_request != DrawRequest::Skip {
                self.redraw_request = DrawRequest::Skip;
