...
impl Drawable for TitleSoundPlayer {
    fn draw(&mut self, ctx: &mut ggez::Context) -> ggez::GameResult<()> {
        if self.is_visible() {
...
impl Node2D for TitleSoundPlayer {
    impl_node2d_for_wrapped! {main_text}
}

impl Transform for TitleSoundPlayer {
    impl_transform_object_for_wrapped! {main_text}
}
...
impl UpdatePanel {
    ...
    pub fn check_update<'a>(&mut self, _ctx: &mut SuzuContext<'a>, _t: Clock) -> Result<String, ()> {
        let resp = match reqwest::blocking::get("https://boxed-sumire.fun/suzu/update_2x.toml") {
...
pub struct Gallery {
    ...
    header_text: UniText,
    event_list: DelayEventList<Self>,
    gallery_list: Vec<(Texture, String)>,
...
impl Gallery {
    ...
    pub fn current_gallery_image(&mut self) -> Option<&mut dyn Drawable> {
        ...
        match index {
            ...
            5 | 6 => {
                ...
                Some(&mut self.char_list.get_mut(index).unwrap().0 as &mut dyn Drawable)
            }
            _ => None,
        }
    }
...
impl Drawable for TitleContents {
    fn draw(&mut self, ctx: &mut ggez::Context) -> ggez::GameResult<()> {
        match self {
