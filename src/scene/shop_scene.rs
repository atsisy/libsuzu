...
use crate::libsuzu::{device as tdev, sound::SoundPlayFlags};

use ggez::graphics as ggraphics;
use crate::libsuzu::core::Clock;
use crate::libsuzu::core::Updatable;
...
impl Drawable for CharacterGroup {
    #[inline(always)]
...
impl Drawable for CustomerQueue {
    fn draw(&mut self, ctx: &mut ggez::Context) -> ggez::GameResult<()> {
        if self.is_visible() {
...
impl Drawable for GoToCheckCustomers {
    fn draw(&mut self, ctx: &mut ggez::Context) -> ggez::GameResult<()> {
        if self.is_visible() {
...
impl ShopScene {
    ...
    fn try_hide_storing_select_ui<'a>(&mut self, ctx: &mut SuzuContext<'a>, t: Clock) {
	    ...
	    ctx.take_save_data_mut().suzuna_book_pool.push_book_vec(stored);
