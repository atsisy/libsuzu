...
impl SuzunaSubScene {
	...
	let todays_new_books = match ctx.take_save_data().game_mode {
	    GameMode::Story => DayNewBooks::random(ctx.resource, 7, 2),
	    GameMode::TimeAttack(_) => DayNewBooks::random(ctx.resource, 8, 2),
	};
